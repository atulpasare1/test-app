         ...
         <form class="m-5">
                  ...
                  <div class="col-sm-4">
                     <div class="form-floating form-floating-outline">
                        <select class="form-select select2" name="board" id="floatingSelectBorad" aria-label="Floating label select example">
                          <option value="" disabled selected>Select a state board</option>
                          @forelse ($boards as $item)
                          <option value="{{ $item->board_id }}">{{ $item->board_name }}</option>
            ...
            <div class="mb-8 float-end">
              ...
              <script>
                ...
                $('#floatingSelectSubject').change(function(e) {
                            var subject_id = $(this).val(); // Get selected class ID
                            //alert(subject_id);
                            // Check if a class is selected
                            if (subject_id) {
                                // Send AJAX request to get subjects for the selected class
                                $.ajax({
                                    url: '{{ route("get.lessions.by.subjects") }}',
                                    type: 'POST',
                                    data: {
                                    ...
                                    success: function(response) {
                                        ...
                                        $('select[name="lession"]').empty();
                                        
                                        // Add a default 'Select Subject' option
                                        $('select[name="lession"]').append('<option value="">Select Subject</option>');
                                        
                                        // Loop through subjects and append them to the dropdown
                                        $.each(response.lessions, function(index, lession) {
                                            $('select[name="lession"]').append('<option value="' + lession.lesson_id + '">' + lession.lesson_name + '</option>');
                                        });
                                    },
                                ...
                                // If no class is selected, clear the subjects dropdown
                                $('select[name="lession"]').empty();
                                $('select[name="lession"]').append('<option value="">Select Lession</option>');
                            }
                        });
...
$('select#floatingSelectSubject').change(function() {
            var subject_id = $(this).val(); // Get selected class ID
            ...
            if (subject_id) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        // Clear previous subjects
                        $('#floatingSelectLession').empty();
                        
                        // Add a default 'Select Subject' option
                        $('#floatingSelectLession').append('<option value="">Select Subject</option>');
                        
                        // Loop through subjects and append them to the dropdown
                        $.each(response.lessions, function(index, lession) {
                          console.log(lession.lesson_name);
                            $('#floatingSelectLession').append('<option value="' + lession.lesson_id + '">' + lession.lesson_name + '</option>');
                        });
                    },
                    error: function(xhr, status, error) {
                        // Handle errors if any
                ...
                // If no class is selected, clear the subjects dropdown
                $('#floatingSelectLession').empty();
                $('#floatingSelectLession').append('<option value="">Select Lession</option>');
            }
        });
...
// Append the Quill content to the FormData
    formData.append('description', quillContent);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                  createToaster('success', 'Quiz created successfully!');
